...
void Settings::ApplyStyle()
{
  ...
  if (style_type == StyleType::User)
  {
    ...
    {
      const QString stylesheet_contents{QString::fromUtf8(stylesheet.readAll().data())};
    ...
    // If user stylesheet is empty or cannot be opened, let it fall back to the system style.
  }
  ...
  {
    // Default-constructed palette is used.
  }
...

  {
    ...
    QColor border_color;
    GetToolTipStyle(window_color, text_color, unused_text_emphasis_color, border_color, palette, palette);
